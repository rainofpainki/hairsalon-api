...
(
  STYLELIST_ID NUMBER NOT NULL
, SHOP_ID NUMBER NOT NULL
, STYLELIST_NAME VARCHAR2(20) NOT NULL
, STYLELIST_MESSAGE VARCHAR2(500)
, STYLELIST_THUMB_URL VARCHAR2(200)
, CONSTRAINT STYLELIST_PK PRIMARY KEY
  (
    STYLELIST_ID
  )
  ENABLE
...
ADD CONSTRAINT STYLELIST_SHOP_FK FOREIGN KEY
(
  SHOP_ID
)
REFERENCES SHOP
...
COMMENT ON COLUMN STYLELIST.STYLELIST_ID IS '스타일리스트 ID';
COMMENT ON COLUMN STYLELIST.SHOP_ID IS '헤어샵 ID';
COMMENT ON COLUMN STYLELIST.STYLELIST_NAME IS '스타일리스트 이름';
COMMENT ON COLUMN STYLELIST.STYLELIST_MESSAGE IS '스타일리스트 소개 메시지';
COMMENT ON COLUMN STYLELIST.STYLELIST_THUMB_URL IS '스타일리스트 썸네일 URL';
...
ALTER TABLE RESERVATION
ADD CONSTRAINT RESERVATION_STYLELIST_FK FOREIGN KEY
(
  STYLELIST_ID
)
...
COMMENT ON COLUMN RESERVATION.STYLELIST_ID IS '스타일리스트 ID';
...
CREATE SEQUENCE STYLELIST_SEQ INCREMENT BY 1;
